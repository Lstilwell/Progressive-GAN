...
def train_progressive_gan(
    ...
    resume_run_id           ="/home/data/results_128/000-pgan-celebahq-preset-v2-1gpu-fp32/network-snapshot-002705.pkl",         # Run ID or network pkl to resume training from, None = start from scratch.
    ...
    with tf.device('/gpu:0'):
        if resume_run_id is not None:
            ...
            G = tfutil.Network('G', num_channels=training_set.shape[0], resolution=128, label_size=training_set.label_size, **config.G)
            D = tfutil.Network('D', num_channels=training_set.shape[0], resolution=128, label_size=training_set.label_size, **config.D)
            Gs = G.clone('Gs')
